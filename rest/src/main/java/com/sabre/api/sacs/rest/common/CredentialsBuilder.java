...
public class CredentialsBuilder {
	...
	public String getCredentialsString() {
		/*
		 * 从配置文件读取配置拼接冒号,生成授权需要的头信息
		 */
		StringBuilder credentials = new StringBuilder();

		credentials.append(config.getRestProperty("formatVersion")).append(":")
				.append(config.getRestProperty("userId")).append(":").append(config.getRestProperty("group"))
				.append(":").append(config.getRestProperty("domain"));
