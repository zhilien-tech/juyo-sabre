...
public class BargainFinderMaxSoapActivity implements Activity {
	...
	private OTAAirLowFareSearchRQ request;

	public void setRequest(OTAAirLowFareSearchRQ request) {
	...
	public Activity run(SharedContext context) {
		...
		try {
			...
			if (result.getErrors() != null && result.getErrors().getError() != null
					&& !result.getErrors().getError().isEmpty()) {
	...
	private OTAAirLowFareSearchRQ getRequestBody() {
		...
		rq.getOriginDestinationInformation().add(odi);
		...
		rq.setTravelPreferences(travelPreferences);
		...
		rq.setTravelerInfoSummary(tiSummaryType);
